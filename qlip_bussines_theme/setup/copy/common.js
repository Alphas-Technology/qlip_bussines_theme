...
frappe.avatar = function (user, css_class, title, image_url = null) {
	...
	if (user_info.image || image_url) {
		...
		if (css_class === 'avatar-small' || css_class == 'avatar-xs') {
			...
			html_avatar = `<i class="fa fa-user-circle-o fa-2x li-icon-user user" aria-hidden="true"></i>`;
		}
		else{
			html_avatar = `<div class="standard-image" style="background-color: ${user_info.color};">
					${abbr}</div>`;
		}
		return `<span class="avatar ${css_class}" title="${title}">
				${html_avatar}
